...
def plot_reward_text(Tot_Episodes, Tot_Reward, Text):
    #Start Plotting
    ...
    plt.title('Reward Result - ' + Text, fontsize=15)
    plt.ylabel('Total Reward [num]', fontsize=15)
    plt.xlabel('Episode [num]', fontsize=20)
    ax.grid(True)
    plt.show()

def plot_legend_text(Time_Best, Light_Best, Light_Feed_Best, Action_Best, r_Best, perf_Best, SC_Best, SC_Best_norm_hist, SC_Feed_Best, Occup_Best, Text):
    #Start Plotting
    fig, ax = plt.subplots(1)
    fig.autofmt_xdate()
    ...
    legend = ax.legend(loc='center right', shadow=True)
    plt.legend(loc=9, prop={'size': 10})
    plt.title('Best Experimental Result - ' + Text, fontsize=15)
